...
public class UpperBoundParallelTest {
    ...
    public void justASimpleSmokeTest(TestContext testContext) {
        ...
        target.request(limit, -1, null, new UpperBoundParallel.Mentor<Void>() {
    ...
    public void worksIfHandlerThrows(TestContext testContext) {
        ...
        target.request(limiter, -1, null, new UpperBoundParallel.Mentor<Void>() {
    ...
    public void testSemaphoreAreAllReleasedBeforeOnDoneCall(TestContext testContext) {
        ...
        parallel.request(limiter, -1, null, new UpperBoundParallel.Mentor<>() {
    ...
    public void worksIfHandlerThrowsAndContinue(TestContext testContext) {
        Async async = testContext.async();
        Throwable myFancyTestThrowable = new Throwable("Throwable") {
        };
        RuntimeException myFancyTestException = new RuntimeException("RuntimeException") {
        };
        final int totalPermits = 10;
        final int permitsUsedByOther = 2;
        Semaphore limiter = new Semaphore(totalPermits);
        ...
        int availTokens = limiter.availablePermits();
        target.request(limiter, -1, null, new UpperBoundParallel.Mentor<Void>() {
            Iterator<String> iter = List.of("the-lonely-elem-1", "the-lonely-elem-2", "the-lonely-elem-3", "the-lonely-elem-4", "the-lonely-elem-5", "the-lonely-elem-6", "the-lonely-elem-7", "the-lonely-elem-8").iterator();

            @Override
            public boolean runOneMore(BiConsumer<Throwable, Void> onDone, Void unused) throws Throwable {
                if (iter.hasNext()) {
                    String value = iter.next();
                    if ("the-lonely-elem-2".equals(value)) {
                        throw myFancyTestThrowable;
                    } else if ("the-lonely-elem-4".equals(value)) {
                        throw myFancyTestException;
                    } else if ("the-lonely-elem-6".equals(value)) {
                        vertx.runOnContext((Void v) -> { // <- Just imagine some async operation here
                            onDone.accept(new NoStackTraceThrowable("NoStackTraceThrowable"), null);
                        });
                    } else {
                        vertx.runOnContext((Void v) -> { // <- Just imagine some async operation here
                            onDone.accept(null, null);
            ...
            @Override
            public boolean onError(Throwable ex, Void ctx) {
                testContext.assertTrue(myFancyTestException == ex || myFancyTestThrowable == ex || ex.getMessage().equals("NoStackTraceThrowable"));
                return true;
            }
    ...
    @Test
    public void worksIfHandlerThrowableAndStop(TestContext testContext) {
        Async async = testContext.async();
        final int totalPermits = 10;
        final int permitsUsedByOther = 2;
        Semaphore limiter = new Semaphore(totalPermits);
        ...
        target.request(limiter, -1, null, new UpperBoundParallel.Mentor<Void>() {
            ...
            public boolean runOneMore(BiConsumer<Throwable, Void> onDone, Void unused) {
                if (iter.hasNext()) {
                    String value = iter.next();
                    if ("the-lonely-elem-3".equals(value)) {
                        vertx.runOnContext((Void v) -> { // <- Just imagine some async operation here
                            onDone.accept(new Throwable("TestThrowable"), null);
                        });
                    } else {
                        vertx.runOnContext((Void v) -> { // <- Just imagine some async operation here
                            try {
            ...
            @Override
            public boolean onError(Throwable ex, Void ctx) {
                testContext.assertTrue(ex.getMessage().equals("TestThrowable"));
                vertx.setTimer(100, nonsense -> {
                    // all permits I took should be released, but permits from other will not
                    testContext.assertEquals(totalPermits - permitsUsedByOther, limiter.availablePermits());
                    testContext.assertEquals(availTokens, limiter.availablePermits());
                    async.complete();
    ...
    @Test
    public void waitAndRetryIfNoTokensLeft(TestContext testContext) {
        Async async = testContext.async();
        ...
        int delayWaitIfResourceExhaustion = 1000;
        target.request(limiter, delayWaitIfResourceExhaustion, null, new UpperBoundParallel.Mentor<Void>() {
            Iterator<String> iter = List.of("the-lonely-elem-1", "the-lonely-elem-2", "the-lonely-elem-3", "the-lonely-elem-4", "the-lonely-elem-5", "the-lonely-elem-6", "the-lonely-elem-7", "the-lonely-elem-8").iterator();
